...
<div class="space-y-6">
  ...
  {:else if collectionsStore.currentCollection}
        ...
        <div class="p-6">
          <div class="flex items-center justify-between">
            <div class="flex-1">
              ...
              <p class="mt-2 text-3xl font-semibold text-secondary-900">
                {isLoadingStats ? '...' : (collectionsStore.currentStats?.indexes ?? '0')}
              </p>
            </div>
        ...
        <div class="p-6">
          <div class="flex items-center justify-between">
            <div class="flex-1">
              ...
              <p class="mt-2 text-3xl font-semibold text-secondary-900">
                {isLoadingStats ? '...' : `${collectionsStore.currentStats?.size ?? 0} KB`}
              </p>
            </div>
        ...
        <dl class="space-y-4">
          ...
          <div>
            ...
            <dd class="mt-1 text-sm text-secondary-900">
              {formatRelativeTime(collectionsStore.currentCollection.created_at)}
            </dd>
          </div>
          ...
          <div>
            ...
            <dd class="mt-1 text-sm text-secondary-900">
              {formatRelativeTime(collectionsStore.currentCollection.updated_at)}
            </dd>
          </div>
    ...
    {#if collectionsStore.currentCollection.schema}
      <Card title="Schema">
        <pre class="bg-secondary-50 p-4 rounded-lg overflow-x-auto text-sm">
          <code>{formatJSON(collectionsStore.currentCollection.schema)}</code>
        </pre>
      </Card>
    {/if}
    ...
    {#if collectionsStore.currentCollection.indexes}
      <Card title="Indexes">
        <pre class="bg-secondary-50 p-4 rounded-lg overflow-x-auto text-sm">
          <code>{formatJSON(collectionsStore.currentCollection.indexes)}</code>
        </pre>
      </Card>
    {/if}
    ...
    {#if collectionsStore.currentCollection.options}
      <Card title="Options">
        <pre class="bg-secondary-50 p-4 rounded-lg overflow-x-auto text-sm">
          <code>{formatJSON(collectionsStore.currentCollection.options)}</code>
        </pre>
      </Card>
    {/if}
  {:else}
